...
class Algo(ABC):
    ...
    def in_map_quarante_deux(self, new_x, new_y):
        r = int((self.materials['height'] / 2)) - 2
        c = int((self.materials['width'] / 2)) - 3
        if new_y == r and new_x in [c, c + 4, c + 5, c + 6]:
            return True
        elif new_y == r + 1 and new_x in [c, c + 6]:
            return True
        elif new_y == r + 2 and new_x in [c, c + 1, c + 2,  c + 4, c + 5, c + 6]:
            return True
        elif new_y == r + 3 and new_x in [c + 2, c + 4]:
            return True
        elif new_y == r + 4 and new_x in [c + 2, c + 4, c + 5, c + 6]:
            return True
        else:
            return False
